...
jpostdb.protein_browser = jpostdb.protein_browser || {
	...
	var svg = view.append("svg")
      ...
      jpostdb.protein_browser.param.uniprot = stanza_params.uniprot;
      jpostdb.protein_browser.param.dataset = stanza_params.dataset;
	var url = "https://tools.jpostdb.org/subdb/covid-19/json/" + stanza_params.uniprot + ".json";
	...
	var group = jpostdb.protein_browser;
      ...
      if (json.seq.length * 10 < param.width) {
	param.width = json.seq.length * 10;
	...
	group.mouseEvent(svg, setData, plot);
      ...
      var url = "https://tools.jpostdb.org/subdb/covid-19/json/" + param.dataset + "_" + param.uniprot + ".json";
    ...
    psm_align: function(json, renderDiv, renderId){
	var group = jpostdb.protein_browser;
	var param = group.param;
      var svg = renderDiv.select("#protein_browser_svg");
	...
	let obj = {
	  pep_id: d.sequence,
	  begin: Number(d.position),
	  end: Number(d.position) + d.sequence.length - 1,
	  uniq: 0,
	  color: "#f39800",
	  y: 1,
	  count: d.psms,
	  label: d.sequence + ": " + d.psms
	}

	if (d.mutation) {
	  mut_flag = true;
	  let mut_array = [];
	  for (let dat of d.mutation.split(/,/)) {
	    let tmp = dat.match(/([A-Z\*])(\d+)([A-Z\*])/);
	    mut_array.push({
              shift: 0,
              mt_begin: tmp[2],
              mt_end: tmp[2],
	...
	let uniq = 0;
	mut[i].y = y_axis;
	for (let j = 0; j < mut[i].mtd.length; j++) {
	  mut[i].mtd[j].y = y_axis;
	}
      }
      console.log(mut[0].y);
	...
	plot();
      ...
      if (mut_flag) group.mutated_peptide(mut, renderDiv, renderId);
